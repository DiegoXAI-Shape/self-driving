...
from models.modules.BEV_planning import PolynomialSmoothnessLoss
from models.dataset import CARLADataset, compute_planning_metrics, compute_temporal_metrics_complete, EarlyStopping


def train_exp3(args):
    """
    Main training pipeline for Experiment No. 3 (DINOv2 + Quintic Polynomial Head + Kinematic Smoothness Loss).
    Guarantees mathematically smooth trajectories and exact heading alignment via analytical tangent derivation.
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"[Device] Training Experiment 3 on: {device} (Polynomial Trajectory Head)")
    ...
    print(f"[Model] Initializing Experiment 3 BEVPerceptionNetV2 with Quintic Polynomial Head...")
    ...
    smooth_criterion = PolynomialSmoothnessLoss(w_accel=0.1, w_jerk=0.05, w_curvature=0.01)
    ...
    if args.resume:
        ...
        if os.path.exists(ckpt_path):
            print(f"[Resume] Resuming Experiment 3 training from: {ckpt_path}")
    ...
    for epoch in range(start_epoch, args.epochs):
        print(f"\n--- Epoch {epoch+1}/{args.epochs} (Experiment 3: Quintic Polynomial + Smoothness Loss) ---")
        
        # ── Train ──
        model.train()
        train_loss = 0.0
        train_huber_loss = 0.0
        ...
        train_accel = 0.0
        train_yaw = 0.0
        train_bar = tqdm(train_loader, desc="Training Exp3")
        optimizer.zero_grad()
        ...
        for batch_idx, (camera_imgs, lidar_bev, extrinsics, intrinsics, target_waypoints) in enumerate(train_bar):
            ...
            pred_wps, (cx, cy) = model(camera_imgs, lidar_bev, extrinsics, intrinsics)
            
            loss_huber = huber_criterion(pred_wps, target_waypoints)
            loss_smooth = smooth_criterion(cx, cy)
            total_loss = loss_huber + loss_smooth
            
            loss_accum = total_loss / args.accumulation_steps
            loss_accum.backward()
            ...
            train_huber_loss += loss_huber.item()
            ...
            train_bar.set_postfix({
                ...
                "ADE": f"{ade_b:.2f}m",
                "FDE": f"{fde_b:.2f}m",
                "YawErr": f"{temp_m['yaw_error_deg']:.1f}°"
            })
        ...
        val_accel = 0.0
        val_yaw = 0.0
        val_bar = tqdm(val_loader, desc="Validating Exp3")
        ...
        print(f"Epoch {epoch+1:02d} Summary: Loss Train: {epoch_train_loss:.4f} | Loss Val: {epoch_val_loss:.4f} | ADE: {epoch_val_ade:.2f}m | FDE: {epoch_val_fde:.2f}m | YawErr: {epoch_val_yaw:.1f}° | LR: {current_lr:.6e}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Experiment 3: DINOv2 + Quintic Polynomial Head + Kinematic Smoothness Loss")
    ...
    parser.add_argument("--stride",             type=int, default=10, help="Stride step between sequences")
    ...
    parser.add_argument("--model_dir",          default="./checkpoints/experimento_3/", help="Output directory for Experiment 3 checkpoints")
    ...
    print("\n" + "="*60)
    print("  HELIOSKRILL — EXPERIMENT 3 (Quintic Polynomial + Kinematic Smoothness)")
    print("="*60)
